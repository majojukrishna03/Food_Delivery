...
const Cart = () => {
  ...
  return (
    <>
      <Header user = {localStorage.getItem('user')} token = {localStorage.getItem('token')} cartCount={cart.reduce((count, item) => count + item.quantity, 0)} /> {/* Pass total quantity to Header */}
      <div className='background'>
        <div className="cart-container">
          <h2>Your Cart</h2>
          {cart.length === 0 ? (
